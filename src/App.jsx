...
export default function App() {
  ...
  const [selectedSensor, setSelectedSensor] = useState('S-003')
  ...
  const pagePanel = () => {
    ...
    if (activePage === 'AI Control') return <div><h3>AI Model Status</h3><p>Confidence: 92%</p><p>Predicted O₂(6h): 8.1%</p><p>Predicted Degradation: 0.71 1/day</p><p>Risk Score: 2.3/10</p><ul>{recommendations.map(r=><li key={r}>{r}</li>)}</ul><button onClick={applyAI}>Apply Recommendation</button><h4>What-if Analysis</h4><p>+12% aeration yields +0.4 O₂ and -8% thermal risk.</p></div>
    if (activePage === 'Equipment') return <div><h3>Equipment Status</h3>{equipment.map(e=><div key={e.id} className='panel-row' onClick={()=>setSelectedSensor(e.id)}><b>{e.name}</b><span>{e.status}</span></div>)}<p>Communication Health: Gateway Connected (99.8%).</p></div>
  ...
  return <div className='app-shell'>
    ...
    <main className={`main-grid ${viewMode==='Dashboard View'?'dash':''} ${viewMode==='3D View'?'only3d':''}`}>
      ...
      <section className='center glass'>
        <DigitalTwinScene page={activePage} activeLayer={activeLayer} selectedZone={selectedZone} setSelectedZone={setSelectedZone} setSelectedSensor={setSelectedSensor} sensors={sensors} controlPulse={controlPulse} />
      ...
      {viewMode!=='3D View' && <aside className='glass side-right'>{pagePanel()}<div className='mini'><ResponsiveContainer width='100%' height={180}><LineChart data={series}><XAxis dataKey='time' stroke='#87a7bb' /><YAxis stroke='#87a7bb' /><Tooltip /><Line dataKey='oxygen' stroke={activeLayer==='Temperature'?'#ff7a45':'#00D9FF'} /><Line dataKey='temperature' stroke={activeLayer==='Microbial'?'#42ff8f':'#FFB020'} /></LineChart></ResponsiveContainer></div></aside>}
    </main>
